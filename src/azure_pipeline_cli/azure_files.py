...
AgentTransferShell = Literal["powershell", "bash"]
...
def build_agent_transfer_script(command: AzureFilesCommand, config: AzureFilesConfig, shell: AgentTransferShell = "powershell") -> str:
    """Render an Azure Files REST transfer script for the selected agent shell."""

    if shell == "bash":
        return build_agent_transfer_bash_script(command, config)
    return build_agent_transfer_powershell_script(command, config)


def build_agent_transfer_powershell_script(command: AzureFilesCommand, config: AzureFilesConfig) -> str:
    """Render a PowerShell Azure Files REST transfer for a Windows agent."""
...
def build_agent_transfer_bash_script(command: AzureFilesCommand, config: AzureFilesConfig) -> str:
    """Render a Bash/Python Azure Files REST transfer for a Linux or macOS agent."""
    ...
    return f"""set -e
export EVILAZP_AF_ACTION={shlex.quote(command.action)}
export EVILAZP_AF_ACCOUNT={shlex.quote(config.account)}
export EVILAZP_AF_SAS={shlex.quote(config.sas)}
export EVILAZP_AF_SHARE={shlex.quote(remote.share)}
export EVILAZP_AF_REMOTE_ROOT={shlex.quote(remote.path)}
export EVILAZP_AF_LOCAL_PATH={shlex.quote(local_path)}
export EVILAZP_AF_OVERWRITE={shlex.quote(str(command.overwrite).lower())}
export EVILAZP_AF_REMOTE_ROOT_IS_DIRECTORY_TARGET={shlex.quote(str(command.action == "upload" and _remote_path_is_directory_target(command.destination)).lower())}
export EVILAZP_AF_LOCAL_PATH_IS_DIRECTORY_TARGET={shlex.quote(str(command.action == "download" and _local_path_is_directory_target(command.destination)).lower())}
python3 - <<'PY'
import email.utils
import os
import posixpath
...
ACTION = os.environ["EVILAZP_AF_ACTION"]
ACCOUNT = os.environ["EVILAZP_AF_ACCOUNT"]
SAS = os.environ["EVILAZP_AF_SAS"].lstrip("?")
SHARE = os.environ["EVILAZP_AF_SHARE"]
REMOTE_ROOT = os.environ["EVILAZP_AF_REMOTE_ROOT"]
LOCAL_PATH = os.environ["EVILAZP_AF_LOCAL_PATH"]
OVERWRITE = os.environ["EVILAZP_AF_OVERWRITE"].lower() == "true"
REMOTE_ROOT_IS_DIRECTORY_TARGET = os.environ["EVILAZP_AF_REMOTE_ROOT_IS_DIRECTORY_TARGET"].lower() == "true"
LOCAL_PATH_IS_DIRECTORY_TARGET = os.environ["EVILAZP_AF_LOCAL_PATH_IS_DIRECTORY_TARGET"].lower() == "true"
VERSION = "2022-11-02"
...
def join_remote_path(base, child):
...
def uri(path, extra_query=""):
    encoded_share = urllib.parse.quote(SHARE, safe="")
    encoded_path = encode_path(path)
    url = f"https://{{ACCOUNT}}.file.core.windows.net/{{encoded_share}}"
    if encoded_path:
...
def request(method, path, extra_query="", extra_headers=None, data=None, outfile=None):
    req = urllib.request.Request(uri(path, extra_query), method=method, headers=headers(extra_headers))
    try:
        ...
        raise RuntimeError(f"Azure Files HTTP {{exc.code}} for {{method}} /{{SHARE}}/{{path}}: {{body}}") from exc


def remote_file_exists(path):
    req = urllib.request.Request(uri(path), method="HEAD", headers=headers())
    try:
        ...
        if exc.code == 404:
            return False
        body = exc.read().decode("utf-8", errors="replace")
        raise RuntimeError(f"Azure Files HTTP {{exc.code}} for HEAD /{{SHARE}}/{{path}}: {{body}}") from exc
...
def ensure_directory(path):
    ...
    for part in [item for item in path.split("/") if item]:
        ...
        try:
            request("PUT", current, "restype=directory")
...
def upload_file(source_path, remote_path):
    if os.path.isdir(source_path):
        raise RuntimeError(f"local path is not a file: {{source_path}}")
    if remote_file_exists(remote_path) and not OVERWRITE:
        raise RuntimeError(f"remote file already exists: {{remote_path}}")
    ensure_parent_directory(remote_path)
    length = os.path.getsize(source_path)
    request("PUT", remote_path, extra_headers={{"x-ms-type": "file", "x-ms-content-length": str(length)}})
    if length == 0:
        return
    offset = 0
...
def upload_path():
    ...
    effective_remote_root = REMOTE_ROOT
    if REMOTE_ROOT_IS_DIRECTORY_TARGET:
        effective_remote_root = join_remote_path(REMOTE_ROOT, os.path.basename(os.path.normpath(LOCAL_PATH)))
    if os.path.isdir(LOCAL_PATH):
        for root, _dirs, files in os.walk(LOCAL_PATH):
            for name in sorted(files):
                source = os.path.join(root, name)
                relative = os.path.relpath(source, LOCAL_PATH).replace(os.sep, "/")
                upload_file(source, join_remote_path(effective_remote_root, relative))
    else:
        upload_file(LOCAL_PATH, effective_remote_root)
    print(f"[upload] {{LOCAL_PATH}} -> /{{SHARE}}/{{effective_remote_root}}")


def list_directory(path):
    try:
        data = request("GET", path, "restype=directory&comp=list")
    except RuntimeError as exc:
        if "HTTP 404" in str(exc):
            raise RuntimeError(f"remote path not found: /{{SHARE}}/{{path}}") from exc
        raise
    ...
    if entries is not None:
        for item in entries.findall("Directory"):
            name = item.findtext("Name")
            if name:
                directories.append(name)
        for item in entries.findall("File"):
            name = item.findtext("Name")
            if name:
                files.append(name)
...
def download_file(remote_path, destination_path):
    if os.path.exists(destination_path) and not OVERWRITE:
        raise RuntimeError(f"local file already exists: {{destination_path}}")
    parent = os.path.dirname(destination_path)
    if parent:
        os.makedirs(parent, exist_ok=True)
    request("GET", remote_path, outfile=destination_path)


def download_directory(remote_path, destination_root):
    directories, files = list_directory(remote_path)
    count = 0
    for name in directories:
        count += download_directory(join_remote_path(remote_path, name), os.path.join(destination_root, name))
    for name in files:
        download_file(join_remote_path(remote_path, name), os.path.join(destination_root, name))
        count += 1
    if count == 0:
...
def download_path():
    if remote_file_exists(REMOTE_ROOT):
        destination = LOCAL_PATH
        if os.path.isdir(destination) or LOCAL_PATH_IS_DIRECTORY_TARGET:
            leaf = [part for part in REMOTE_ROOT.split("/") if part][-1]
            destination = os.path.join(destination, leaf)
        download_file(REMOTE_ROOT, destination)
    else:
        download_directory(REMOTE_ROOT, LOCAL_PATH)
    print(f"[download] /{{SHARE}}/{{REMOTE_ROOT}} -> {{LOCAL_PATH}}")
...
def format_agent_transfer_display_command(command: AzureFilesCommand) -> str:
    """Return a short, secret-free command label for an agent-side transfer."""

    parts = [f"/{command.action}", "--target", "agent"]
    if command.overwrite:
        parts.append("--overwrite")
    parts.extend([command.source, command.destination])
