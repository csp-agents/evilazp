...
def test_bare_help_requires_slash(capsys):
    from azure_pipeline_cli.shell import PipelineShell
...
def test_banner_uses_evilazp_brand_and_current_version():
    ...
    assert "1s live cache" in banner
...
def test_bare_shell_commands_require_slash(capsys):
    from azure_pipeline_cli.shell import PipelineShell

    shell = PipelineShell(None)

    assert shell.onecmd("help") is None
    assert shell.onecmd("clear") is None
    assert shell.onecmd("exit") is None

    captured = capsys.readouterr()
    assert "Did you mean /help?" in captured.err
    assert "Did you mean /clear?" in captured.err
    assert "Did you mean /exit?" in captured.err


def test_slash_help_still_dispatches_locally(capsys):
    from azure_pipeline_cli.shell import PipelineShell
...
def test_unknown_slash_command_does_not_execute_remotely(monkeypatch, capsys):
    from azure_pipeline_cli.shell import PipelineShell

    shell = PipelineShell(None)
    calls = []

    monkeypatch.setattr(shell, "_execute_remote_command", lambda line: calls.append(line))

    assert shell.onecmd("/rum whoami") is None

    captured = capsys.readouterr()
    assert "unknown slash command: /rum" in captured.err
    assert calls == []


def test_ctrl_c_message_points_to_slash_exit(monkeypatch, capsys):
    from azure_pipeline_cli.shell import PipelineShell

    shell = PipelineShell(None)

    def raise_interrupt(_line):
        raise KeyboardInterrupt

    monkeypatch.setattr(shell, "default", raise_interrupt)

    assert shell.onecmd("whoami") is None

    captured = capsys.readouterr()
    assert "Use /exit to quit." in captured.out


def test_preloop_clears_screen_only_once(monkeypatch):
    ...
    monkeypatch.setattr(shell_module, "clear_screen_on_start", lambda: calls.append("clear"))
    monkeypatch.setattr(shell, "_start_agent_sync", lambda: calls.append("sync"))

    shell.preloop()
    shell.preloop()

    assert calls == ["clear", "sync"]
...
def test_whoami_runs_remotely_instead_of_quick_local_lookup(capsys):
    from azure_pipeline_cli.models import AgentRef
    from azure_pipeline_cli.shell import PipelineShell
...
def test_run_whoami_executes_remote_command(capsys):
    ...
    class FakeClient:
        ...
        def queue_pipeline_command(self, pipeline, agent, command_b64, run_id):
            self.queued_command = decode_command(command_b64)
            return 123

        def wait_pipeline_command(self, pipeline, build_id, run_id, command_text, timeout, poll_interval):
    ...
    shell.do_run("whoami")
    shell._run_threads[-1].join(timeout=1)

    captured = capsys.readouterr()
    assert client.queued_command == "whoami"
    assert "queued on WINVM01 (build 123)" in captured.out
    assert "[result] WINVM01 build 123 result=succeeded exit=0" in captured.out
...
def test_run_returns_before_background_command_completes(capsys):
    import threading

    from azure_pipeline_cli.azure_devops import decode_command
    from azure_pipeline_cli.models import AgentRef, PipelineRef, RunResult
    from azure_pipeline_cli.shell import PipelineShell, format_prompt
    ...
    class FakeClient:
        ...
        def wait_pipeline_command(self, pipeline, build_id, run_id, command_text, timeout, poll_interval):
            ...
            return RunResult(build_id, "completed", "succeeded", "DONE", 0)

    client = FakeClient()
    shell = PipelineShell(client)
    shell.selected_agent = AgentRef(1, "LINUX01", 10, "azure-pipeline", "online", True)
    shell.selected_pipeline = PipelineRef(7, "host-info", "project")
    shell.prompt = format_prompt("LINUX01")

    shell.do_run("whoami")

    captured = capsys.readouterr()
    assert client.queued_command == "whoami"
    assert "queued on LINUX01 (build 321)" in captured.out
    ...
    captured = capsys.readouterr()
    assert "[result] LINUX01 build 321 result=succeeded exit=0" in captured.out
    assert "DONE" in captured.out
    assert captured.out.rstrip().endswith("evilazp(LINUX01)>")
...
def test_upload_target_agent_executes_remote_azure_files_script(monkeypatch, capsys):
    ...
    captured = capsys.readouterr()
    assert "[result] WINVM01 build 123 result=succeeded exit=0" in captured.out
    assert "$ /upload --target agent --overwrite /tmp/a.txt /share/a.txt" in captured.out
    assert "[upload] /tmp/a.txt -> /share/a.txt" in captured.out
    assert "$ErrorActionPreference" not in captured.out
...
def test_download_target_agent_uses_bash_script_for_linux_agent(monkeypatch, capsys):
    from azure_pipeline_cli.azure_devops import decode_command
    from azure_pipeline_cli.azure_files import AzureFilesConfig
    from azure_pipeline_cli.models import AgentRef, PipelineRef, RunResult
    from azure_pipeline_cli.shell import PipelineShell
    ...
    class FakeClient:
        ...
        def run_pipeline_command(self, pipeline, agent, command_b64, run_id, timeout, poll_interval):
            self.queued_command = decode_command(command_b64)
            return RunResult(123, "completed", "succeeded", "[download] /share/a.txt -> /tmp/a.txt", 0)
    ...
    shell.selected_agent = AgentRef(
        ...
        capabilities={"Agent.OS": "Linux"},
    )
    shell.selected_pipeline = PipelineRef(7, "host-info", "project")

    shell.do_download("--target agent /share/a.txt /tmp/a.txt")

    captured = capsys.readouterr()
    assert "$ /download --target agent /share/a.txt /tmp/a.txt" in captured.out
    assert "[download] /share/a.txt -> /tmp/a.txt" in captured.out
    assert "python3 - <<'PY'" in client.queued_command
    assert "EVILAZP_AF_ACTION=download" in client.queued_command
    assert "Invoke-WebRequest" not in client.queued_command


def test_agent_transfer_shell_detects_linux_from_any_capability():
    from azure_pipeline_cli.models import AgentRef
    from azure_pipeline_cli.shell import agent_transfer_shell
    ...
    agent = AgentRef(
        ...
        "online",
        True,
        capabilities={"Agent.OSDescription": "Ubuntu 22.04 x64"},
    )

    assert agent_transfer_shell(agent) == "bash"


def test_agent_transfer_shell_prefers_windows_marker():
    from azure_pipeline_cli.models import AgentRef
    from azure_pipeline_cli.shell import agent_transfer_shell
    ...
    agent = AgentRef(
        ...
        True,
        os_description="Microsoft Windows",
        capabilities={"Path": "C:\\Windows\\System32"},
    )

    assert agent_transfer_shell(agent) == "powershell"


def test_selected_agent_metadata_refresh_is_cached_for_five_minutes(monkeypatch):
    from azure_pipeline_cli.models import AgentRef, PipelineRef
    from azure_pipeline_cli.shell import PipelineShell
    ...
    class FakeClient:
        def discover_agents(self, pool_name):
        ...
        def bind_agent_to_pipeline(self, agent, pipeline):
            return agent

    shell = PipelineShell(FakeClient())
    shell.selected_agent = AgentRef(1, "agent-app-01", 10, "pool", "online", True)
    shell.selected_pipeline = PipelineRef(7, "pipe", "project")
    monkeypatch.setattr("azure_pipeline_cli.shell.time.monotonic", lambda: 100.0)

    shell._refresh_selected_agent_metadata()
    shell._refresh_selected_agent_metadata()

    assert calls == ["pool"]
    assert shell.selected_agent.os_description == "Linux"
...
def test_use_prepares_runner_variables_and_run_reuses_them(capsys):
    ...
    class FakeClient:
        ...
        def queue_pipeline_command(self, pipeline, agent, command_b64, run_id):
            calls.append(("queue_pipeline_command", pipeline.id, agent.name))
            return 123

        def wait_pipeline_command(self, pipeline, build_id, run_id, command_text, timeout, poll_interval):
            calls.append(("wait_pipeline_command", build_id, command_text))
            return RunResult(build_id, "completed", "succeeded", "ok", 0)
    ...
    assert calls == [
        ...
        ("queue_pipeline_command", 7, "WINVM01"),
        ("wait_pipeline_command", 123, "whoami"),
    ]
...
def test_agent_sync_announces_new_online_agent_after_initial_sync(capsys):
    from azure_pipeline_cli.models import AgentRef
    from azure_pipeline_cli.shell import PipelineShell

    snapshots = [
        [AgentRef(1, "WINVM01", 10, "pool", "online", True)],
        [
            AgentRef(1, "WINVM01", 10, "pool", "online", True),
            AgentRef(
                2,
                ...
                capabilities={"Agent.MachineName": "linux-host"},
            ),
        ],
    ]
    ...
    assert "new agent registered" not in first.out

    shell._sync_agents_once()
    second = capsys.readouterr()
    assert "new agent registered: LINUX01" in second.out
    assert "connected at" in second.out
    assert "from linux-host (Ubuntu 24.04)" in second.out
    assert second.out.rstrip().endswith("evilazp(no-agent)>")
