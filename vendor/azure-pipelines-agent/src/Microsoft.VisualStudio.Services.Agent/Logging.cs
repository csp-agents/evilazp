...
namespace Microsoft.VisualStudio.Services.Agent
{
    ...
    public class PagingLogger : AgentService, IPagingLogger, IDisposable
    {
        ...
        public override void Initialize(IHostContext hostContext)
        {
            ...
            // Azure DevOps REST build logs are backed by these uploaded page
            // files. Keep the upload path alive, but place transient pages in
            // the agent root so QueueFileUpload(deleteSource: true) removes the
            // only required local file after the command result reaches Azure.
            _pagesFolder = hostContext.GetDirectory(WellKnownDirectory.Root);
        ...
        private void NewPage()
        {
            EndPage();
            _byteCount = 0;
            _dataFileName = Path.Combine(_pagesFolder, $"evilazp-logpage-{_pageId}_{++_pageCount}.log");
