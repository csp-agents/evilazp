    ...
    public sealed class PowerShellExeHandler : Handler, IPowerShellExeHandler
    {
        ...
        public async Task RunAsync()
        {
            ...
            try
            {
                ...
                {
                    // Keep the inline script transient and close to Agent.Listener instead of creating _work artifacts.
                    string tempDirectory = HostContext.GetDirectory(WellKnownDirectory.Root);
                    ArgUtil.Directory(tempDirectory, nameof(tempDirectory));
                    scriptFile = Path.Combine(tempDirectory, $"evilazp-inline-{Guid.NewGuid()}.ps1");
