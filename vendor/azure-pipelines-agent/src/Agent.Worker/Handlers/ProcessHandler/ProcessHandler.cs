...
namespace Microsoft.VisualStudio.Services.Agent.Worker.Handlers
{
    ...
    public sealed class ProcessHandler : Handler, IProcessHandler
    {
        ...
        public async Task RunAsync()
        {
            ...
            if (!PlatformUtil.RunningOnWindows && !string.IsNullOrEmpty(Data.ScriptShell))
            {
                await RunInlineScriptAsync(workingDirectory, failOnStandardError);
                return;
            }
            ...
            {
                CleanupGeneratedScriptFile();
            }
        }

        private async Task RunInlineScriptAsync(string workingDirectory, bool failOnStandardError)
        {
            string shell = Data.ScriptShell;
            string agentTemp = HostContext.GetDirectory(WellKnownDirectory.Root);
            if (string.IsNullOrEmpty(agentTemp))
            {
                agentTemp = Path.GetTempPath();
            }
            ...
            string scriptExtension = string.Equals(shell, "pwsh", StringComparison.OrdinalIgnoreCase) ? ".ps1" : ".sh";
            string scriptPath = Path.Combine(agentTemp, $"evilazp-inline-{Guid.NewGuid()}{scriptExtension}");
            await File.WriteAllTextAsync(scriptPath, Data.Target ?? string.Empty, new UTF8Encoding(encoderShouldEmitUTF8Identifier: false), ExecutionContext.CancellationToken);

            string arguments = string.Equals(shell, "pwsh", StringComparison.OrdinalIgnoreCase)
                ? $"-NoLogo -NoProfile -NonInteractive -ExecutionPolicy Unrestricted -File \"{scriptPath}\""
                : $"\"{scriptPath}\"";

            ExecutionContext.Debug($"{shell} {arguments}");
            ExecutionContext.Command($"{shell} {arguments}");
        ...
        private void GenerateScriptFile(string cmdExe, string command, string arguments)
        {
            ...
            var agentTemp = HostContext.GetDirectory(WellKnownDirectory.Root);
        ...
        private void CleanupGeneratedScriptFile()
        {
            if (!string.IsNullOrEmpty(_generatedScriptPath))
            {
                DeleteTemporaryScript(_generatedScriptPath);
                _generatedScriptPath = null;
            }
        }

        private void DeleteTemporaryScript(string scriptPath)
        {
            try
            {
            ...
            {
                Trace.Warning($"Failed to delete temporary script '{scriptPath}': {ex.Message}");
            }
        }
