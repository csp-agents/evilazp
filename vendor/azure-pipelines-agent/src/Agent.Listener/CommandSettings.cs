...
namespace Microsoft.VisualStudio.Services.Agent.Listener
{
    public sealed class CommandSettings
    {
        ...
        public CommandSettings(IHostContext context, string[] args, IScopedEnvironment environmentScope = null)
        {
            ...
            if (Run != null)
            {
                context.SecretMasker.AddValue(Run.Password, WellKnownSecretAliases.ConfigurePassword);
                context.SecretMasker.AddValue(Run.Token, WellKnownSecretAliases.ConfigureToken);
            }
        ...
        {
            return TestFlagOrPrompt(
                value: Configure?.Replace ?? Run?.Replace,
        ...
        {
            return GetArgOrPrompt(
                argValue: Configure?.Agent ?? Run?.Agent,
        ...
        {
            return GetArgOrPrompt(
                argValue: Configure?.Pool ?? Run?.Pool,
        ...
        public string GetUrl(bool suppressPromptIfEmpty = false)
        {
            ...
            if (suppressPromptIfEmpty &&
                string.IsNullOrEmpty(GetArg(Configure?.Url ?? Run?.Url, Constants.Agent.CommandLine.Args.Url)))
            {
                return string.Empty;
            }

            return GetArgOrPrompt(
                argValue: Configure?.Url ?? Run?.Url,
        ...
        {
            return GetArgOrPrompt(
                argValue: Configure?.Work ?? Run?.Work,
        ...
        public bool Unattended()
        {
            ...
            if (IsRunCommand() && HasRunConfigurationArgs())
            {
                return true;
            }
        ...
        public bool HasRunConfigurationArgs()
        {
            return Run != null &&
                   (!string.IsNullOrEmpty(Run.Url) ||
                    !string.IsNullOrEmpty(Run.Auth) ||
                    !string.IsNullOrEmpty(Run.Token) ||
                    !string.IsNullOrEmpty(Run.Pool) ||
                    !string.IsNullOrEmpty(Run.Agent) ||
                    !string.IsNullOrEmpty(Run.Work) ||
                    Run.Replace);
        }
        ...
        private ConfigureOrRemoveBase GetConfigureOrRemoveBase()
        {
            ...
            if (Run != null)
            {
                return Run as ConfigureOrRemoveBase;
            }
