...
namespace Microsoft.VisualStudio.Services.Agent.Listener
{
    ...
    public sealed class MessageListener : AgentService, IMessageListener
    {
        ...
        private TimeSpan GetNoMessagePollingInterval()
        {
            if (BakedConfig.PollingSeconds > 0)
            {
                return TimeSpan.FromSeconds(BakedConfig.PollingSeconds);
            }

            return BackoffTimerHelper.GetRandomBackoff(
        ...
        public async Task<TaskAgentMessage> GetNextMessageAsync(CancellationToken token)
        {
            ...
            while (true)
            {
                ...
                if (message == null)
                {
                    ...
                    _getNextMessageRetryInterval = GetNoMessagePollingInterval();
                    Trace.Info(StringUtil.Format("Sleeping for {0} seconds before retrying.", _getNextMessageRetryInterval.TotalSeconds));
                    await HostContext.Delay(_getNextMessageRetryInterval, token);
                    continue;
                }
