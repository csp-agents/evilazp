...
namespace Microsoft.VisualStudio.Services.Agent.Listener.Configuration
{
    ...
    public sealed class ConfigurationManager : AgentService, IConfigurationManager
    {
        ...
        public async Task ConfigureAsync(CommandSettings command)
        {
            ...
            if (IsConfigured() && command.IsRunCommand() && command.GetReplace())
            {
                Trace.Info("Run command requested --replace. Clearing local agent settings before unattended reconfiguration.");
            ...
            // This fork is intentionally script-only. Avoid installing and advertising
            // broad host tool capabilities that are not required for server connection,
            // job execution, log upload, or result reporting.
            agentSettings.SkipCapabilitiesScan = true;
            // This fork does not use Team Explorer Everywhere. Avoid requiring
            // license.html next to the single-file agent on Linux/macOS.
            agentSettings.AcceptTeeEula = false;
            if (PlatformUtil.HostOS == PlatformUtil.OS.Windows)
            {
...
#pragma warning restore CA1416
            ...
            Trace.Info("Skipping service/autologon configuration. This minimal agent runs as a foreground process only.");
