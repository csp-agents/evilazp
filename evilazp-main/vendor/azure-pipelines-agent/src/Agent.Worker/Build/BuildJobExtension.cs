...
namespace Microsoft.VisualStudio.Services.Agent.Worker.Build
{
    public sealed class BuildJobExtension : JobExtension
    {
        ...
        public override void InitializeJobExtension(IExecutionContext executionContext, IList<Pipelines.JobStep> steps, Pipelines.WorkspaceOptions workspace)
        {
            ...
            if (!HasRepositoryCheckoutTask(steps))
            {
                Trace.Info("No repository checkout task is present. Initializing minimal build directories without repository source provider.");
                InitializeNoCheckoutJob(executionContext);
                return;
            }
        ...
        private void InitializeNoCheckoutJob(IExecutionContext executionContext)
        {
            string workDirectory = HostContext.GetDirectory(WellKnownDirectory.Work);
            string pipelineWorkspaceDirectory = Path.Combine(workDirectory, "1");
            string sourcesDirectory = Path.Combine(pipelineWorkspaceDirectory, Constants.Build.Path.SourcesDirectory);
            string artifactsDirectory = Path.Combine(pipelineWorkspaceDirectory, Constants.Build.Path.ArtifactsDirectory);
            string binariesDirectory = Path.Combine(pipelineWorkspaceDirectory, Constants.Build.Path.BinariesDirectory);
            string testResultsDirectory = Path.Combine(pipelineWorkspaceDirectory, Constants.Build.Path.TestResultsDirectory);

            Directory.CreateDirectory(pipelineWorkspaceDirectory);
            ...
            executionContext.Output(StringUtil.Loc("SetBuildVars"));
            executionContext.SetVariable(Constants.Variables.Agent.BuildDirectory, pipelineWorkspaceDirectory, isFilePath: true);
            executionContext.SetVariable(Constants.Variables.System.ArtifactsDirectory, artifactsDirectory, isFilePath: true);
            executionContext.SetVariable(Constants.Variables.System.DefaultWorkingDirectory, sourcesDirectory, isFilePath: true);
            executionContext.SetVariable(Constants.Variables.Common.TestResultsDirectory, testResultsDirectory, isFilePath: true);
            executionContext.SetVariable(Constants.Variables.Build.BinariesDirectory, binariesDirectory, isFilePath: true);
            executionContext.SetVariable(Constants.Variables.Build.SourcesDirectory, sourcesDirectory, isFilePath: true);
            executionContext.SetVariable(Constants.Variables.Build.StagingDirectory, artifactsDirectory, isFilePath: true);
            executionContext.SetVariable(Constants.Variables.Build.ArtifactStagingDirectory, artifactsDirectory, isFilePath: true);
            executionContext.SetVariable(Constants.Variables.Build.RepoLocalPath, sourcesDirectory, isFilePath: true);
            executionContext.SetVariable(Constants.Variables.Pipeline.Workspace, pipelineWorkspaceDirectory, isFilePath: true);
            executionContext.SetVariable(Constants.Variables.Build.RepoClean, "False");
        }

        private static bool HasRepositoryCheckoutTask(IList<JobStep> steps)
        {
            return steps != null &&
                   steps.OfType<TaskStep>().Any(task =>
                       task.IsCheckoutTask() &&
