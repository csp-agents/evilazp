...
def test_run_pipeline_command_queues_parameters_string(monkeypatch):
    ...
    monkeypatch.setattr(client, "rest_json", fake_rest_json)
    monkeypatch.setattr(client, "wait_for_build", lambda *args: {"status": "completed", "result": "succeeded"})
    monkeypatch.setattr(client, "wait_for_marked_output", lambda *args: ("ok", 0))
