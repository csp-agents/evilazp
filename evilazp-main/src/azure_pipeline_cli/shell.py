...
class PipelineShell(cmd.Cmd):
    ...
    def cmdloop(self, intro: str | None = None) -> bool | None:
        ...
        while True:
            try:
                ...
                print()
                print(warn_text("cancelled; use /exit to quit"))
                next_intro = ""
    ...
    def default(self, line: str) -> bool | None:
        ...
        if first in {
            ...
            print(error_text(f"Unknown remote command '{first}'. Did you mean /{line}?"), file=sys.stderr)
            return None
        if self._print_quick_agent_info(line):
            return None
        return self._execute_remote_command(line)

    def _execute_remote_command(self, line: str) -> bool | None:
        ...
        print(status_text("run", f"{run_id} queued on {self.selected_agent.name}"))
        try:
    ...
    def do_run(self, arg: str) -> bool | None:
        ...
        if not arg.strip():
            ...
            return None
        return self._execute_remote_command(unwrap_outer_quotes(arg))
    ...
    def _execute_agent_azure_files_command(self, command) -> bool | None:
        try:
            ...
            script = build_agent_transfer_script(command, config)
        except AzureFilesError as exc:
            print(error_text(str(exc)), file=sys.stderr)
            return None
        return self._execute_remote_command(script)
    ...
    def _sync_agents_once(self) -> None:
        ...
        with self._agent_lock:
            ...
            if self.selected_agent and not any(
                agent.name == self.selected_agent.name and agent.pool_id == self.selected_agent.pool_id for agent in online_agents
            ):
                self.selected_agent = None
                self.selected_pipeline = None
                self.runner_variables_key = None
...
def format_banner() -> str:
    ...
    hint = style_text("type /help for commands | /agent list is backed by a 5s live cache", "dim")
