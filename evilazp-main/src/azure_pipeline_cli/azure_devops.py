...
class AzureDevOpsClient:
    ...
    def run_pipeline_command(
        ...
        build_id = int(run.get("id") or run.get("buildNumber"))
        ...
        if not output and exit_code is None:
            build = self.wait_for_build(pipeline.project, build_id, timeout, poll_interval)
            output, exit_code, tunnel_urls = self.fetch_marked_output_and_tunnels(
                pipeline.project,
                build_id,
                run_id,
                command_text,
